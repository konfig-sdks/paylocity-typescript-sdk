...
import { readableStreamToString, PaylocityError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                const url = configuration.oauthTokenUrl ?? "https://api.paylocity.com/IdentityServer/connect/token"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new PaylocityError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new PaylocityError(e, e.response?.data)
                }
                if (innerError instanceof PaylocityError) {
                    // Got "PaylocityError" from the above try block
                    throw innerError;
                }
